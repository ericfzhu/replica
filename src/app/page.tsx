		...
		<main className="flex min-h-screen flex-col items-center gap-12 bg-white p-24 text-black">
			<section className="flex max-w-3xl flex-col items-center gap-8">
				...
				<div className="flex gap-2 text-sm">
					...
					<Link
						href="http://data.vision.ee.ethz.ch/ihnatova/public/dped/dped.gz"
						className="border-[1px] border-accent-700 bg-accent-500 px-4 py-1 uppercase text-white duration-300 hover:bg-accent-400"
						target="_blank">
						Dataset
					</Link>
			...
			<section className="flex max-w-3xl flex-col gap-2">
				...
				<p>
					The model is designed to translate low-quality images captured by mobile phones into high-quality images resembling those captured
					by a DSLR camera. The generator network employs convolutional blocks and deconvolutional layers to learn the mapping between the
					mobile and DSLR quality image domains. The discriminator network is a CNN that learns to distinguish between real and fake images
					generated by the generator. The training process involves a two-stage approach, where the generator is trained first to generate
					high-quality images, and then the discriminator is trained to distinguish between real and fake images. The GAN is trained using a
					combination of adversarial loss and L1 loss to encourage the generator to produce images that produce realistic results.
				</p>
				<p>
					Experimental results demonstrate the potential effectiveness of the proposed method in enhancing the visual quality of iPhone 3GS
					images, bringing them closer to the quality of DSLR images.
				</p>
			</section>
